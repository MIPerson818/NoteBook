...
https://blog.csdn.net/pd137/article/details/103766637

集成到cmd/powershell：

1. 在编辑环境变量的窗口中，点击“新建”按钮，并在输入框中输入Git的安装路径。通常情况下，Git的安装路径为```D:\Applications\Git```，重新打开cmd。
2. 在PowerShell窗口中，输入以下命令，将向PowerShell的环境变量中添加Git的安装路径：

```powershell
$env:Path += ";D:\Applications\Git\bin"
```
...
### 1. github的 https clone

1. **获取仓库 HTTPS 地址**：在 GitHub 仓库页面，点击 “Code” 按钮，复制 HTTPS 格式的仓库地址。

2. **克隆仓库**：打开终端（如 Windows 的 Git Bash、Linux 或 macOS 的终端），使用 `git clone` 命令并粘贴刚才复制的地址。

   ``` bash
   git clone <HTTPS 仓库地址>
   ```

3. **输入认证信息**：执行命令后，会提示输入 GitHub 的用户名和密码（现在都使用 2FA，需用个人访问令牌替代密码），输入后即可完成克隆。

   **个人token位置：**

![image-20250221174557274](./assets/image-20250221174557274-1740131159814-1.png)

![image-20250221174706672](./assets/image-20250221174706672-1740131229587-3.png)
